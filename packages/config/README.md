...
- **Semantics:** [`tokens/iptv-semantic-colors.json`](tokens/iptv-semantic-colors.json) — roles map to **`mode.family.step`** (e.g. `light.turquoise.3`, `dark.turquoise.2`).
...
The preset sets **`--iptv-paint-*`** from **`light`** on **`:root`** and overrides them from **`dark`** on **`.dark`**, so **`bg-lum-*`** utilities track the theme without `dark:` prefixes. Semantics use **`--iptv-color-*`** the same way.
